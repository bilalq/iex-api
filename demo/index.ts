...
// import './fetch-ponyfill.d.ts'
// tslint:disable-next-line: no-reference
// / <reference path="fetch-ponyfill.d.ts" />
import fetchNode from '../fetch-ponyfill/fetch-node';
...
wsClient.addSystemEventListener((systemEvent: any) => {
  console.log(`System event: ${util.inspect(systemEvent)}`);
});
wsClient.subscribeSystemEvents();

client.deepSystemEvent().then((event: any) => {
...
const prompt = () => {
  inquirer.prompt(question).then((answers: any) => {
    ...
    client.tops(stock).then((response: any) => {
      console.log(`TOPS http response: ${util.inspect(response)}`);
    });
    client.deep(stock).then((response: any) => {
    ...
    const printDeepResponse = (response: DeepSocketResponse) => {
      console.log(
        `DEEP socket response: ${util.inspect(response, false, null)}`
      );
    };
