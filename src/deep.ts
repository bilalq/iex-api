...
import {
  DEEP_CHANNELS,
  DeepSocketResponse,
  SYSTEM_DEEP_CHANNELS,
  SystemEvent
} from './apis/marketData'
import {
  initExceptionHandlers,
  Socket,
  SocketClientCreator,
  SocketExceptionHandlers,
...
export class DeepService {
  ...
  public constructor(
    socketClientCreator: SocketClientCreator,
  ...
  public subscribeSystemEvents(): void {
    subscribeOnConnected(
      this.socket,
      JSON.stringify({ channels: [SYSTEM_DEEP_CHANNELS.SYSTEM_EVENT] })
    )
  }
  ...
  public removeSystemEventListener(toRemove: SystemEventListener): void {
    this.systemEventListeners = this.systemEventListeners.filter(
      listener => listener !== toRemove
    )
  }
