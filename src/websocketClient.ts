...
export const unsubscribeOnConnected = (
  socket: Socket,
  message: string
): void => {
  if (socket.connected) {
...
export const initExceptionHandlers = (
  socket: Socket,
  exceptionHandlers: SocketExceptionHandlers
): void => {
...
export default class WebsocketIEXClient {
  ...
  private get topsService() {
    if (this.lazyTopsService === null) {
      this.lazyTopsService = new TopsService(
        this.socketClientCreator,
  ...
  private get deepService() {
    if (this.lazyDeepService === null) {
      this.lazyDeepService = new DeepService(
        this.socketClientCreator,
        this.exceptionHandlers,
        this.websocketBaseUrl
      )
  ...
  public constructor(
    socketClient: SocketClientCreator,
    exceptionHandlers: SocketExceptionHandlers = {},
   ...
   */
  public subscribeDeep(
    symbol: string | string[],
    channels = DEFAULT_DEEP_CHANNELS
  ): void {
